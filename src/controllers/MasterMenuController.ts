...
const CreateMenu = async (req: Request, res: Response): Promise<Response> => {
    try {
        ...
        const menu = await MasterMenu.create({
            name, icon, ordering, active: true
        })
...
const UpdateMenu = async (req: Request, res: Response): Promise<Response> => {
    try {
        const { id } = req.params
        const { icon, name, ordering } = req.body

        const menu = await MasterMenu.findOne({ where: { id, active: true } })
...
const PermanentDeleteMenu = async (req: Request, res: Response): Promise<Response> => {
    try {
        const { id } = req.params
        const { icon, name, ordering } = req.body
...
export default { CreateMenu, UpdateMenu, GetAllMenu, GetDetailMenu, GetListMenu, SoftDeleteMenu, PermanentDeleteMenu }